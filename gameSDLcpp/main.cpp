...
bool Init()
{
	...
	g_font_Menu = TTF_OpenFont("COMICATE.TTF", 50); // kich thuoc la 50
...
int main(int arc,char*argv[])
{
	...
	// g_bkground = SDLcommonfunction:: LoadImage("bg600.png");  cach 1: chạy nền lặp lại liên tục
	...
	time_game.set_color(TEXT_OBJECT::BLACK_TEXT);
	...
	//MENU
	unsigned int die_number = 0; // quan li mang cho nhan vat chinh
	...
	{		
		...
		// apply  background
		// bkgn_x -= 2;             // cach 1 : 1 background duoc lap lai 
		/*SDLcommonfunction::ApplySurface(g_bkground, g_screen, bkgn_x, 0);
		SDLcommonfunction::ApplySurface(g_bkground, g_screen, bkgn_x + SCREEN_WIDTH, 0); 
		if (bkgn_x <= -SCREEN_WIDTH)
		{
			bkgn_x = 0; 
		}*/

		// cach 2 : 1 backgroung dài  .............
		...
		for (int ii = 0; ii < NUM_THREAT; ii++)
		{
			...
			if (p_threat != NULL) // con trỏ thì nên phải kiếm tra khác NULL đề phongf chết chương trình 
			{
				 ...
				 if (ii == 2)
				{
				 ...
				 // check va cham giua nhan vat chinh va threat
